...
/**
 * Check if the player's max tech level is equal or larger than 3 for the research cloak order
 */
public class CloakResearchTechLevelChecker extends RuleChecker {
    ...
    protected void checkMyRule(OrderEntity order, PlayerEntity player) throws IllegalArgumentException {
        if(player.getMaxTechLevel()<3){
            throw new IllegalArgumentException("Invalid research cloak order: Player's tech level is less than 3.");
        }
    }
