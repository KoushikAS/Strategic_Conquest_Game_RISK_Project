...
public class EndGameHandlerTest {
    ...
    public void test_Run() throws IOException, InterruptedException {
        ...
        when(mockTestSocket.getInputStream())
                .thenReturn(mockInputStream(new Ack(AckStatusEnum.SUCCESS, "Success")));

        InitialiseGameHandler mockClass = new InitialiseGameHandler(mockTestSocket, getMockGame(2), winningPlayerName);
        mockClass.start();

        mockClass.join();
        byte[] expected = getByteArray(winningPlayerName);
    ...
    public void test_RunWithBadACK() throws IOException, InterruptedException {
        ...
        when(mockTestSocket.getInputStream())
                .thenReturn(mockInputStream(new Ack(AckStatusEnum.FAIL, "Fail")),
                        mockInputStream(new Ack(AckStatusEnum.SUCCESS, "Success")));
